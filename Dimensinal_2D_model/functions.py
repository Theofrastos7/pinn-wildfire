...
def reaction_step(Tn, Sn, dt,A,B,C,CS,Ta):
    ...
    exp_term = np.exp(-B/(Tn - Ta + 1e-8))
    T_new = Tn + dt * A * (Sn * exp_term - C * (Tn - Ta))
...
#by Mandel et al, Math. Comp. Sim. (2008) - no wind present - dimensional equations.
def Diffusion_Reaction_Windless_Operator_Splitting(k,w1,w2,A,B,C,CS,Ta,Nx,Ny,dx,dy,dt,T_total,area_start_x, area_start_y, area_size_x, area_size_y,temp1,temp2,fuel1,fuel2):
