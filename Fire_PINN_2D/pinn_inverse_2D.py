...
class FirePINN():
    def __init__(self, X_data, TF_data, X_collocation, X_ic, TF_ic, layers, lb, ub, fire_params):
        ...
        # Learnable parameters beta, lambda, w1, w2
        ...
        all_params = list(self.dnn.parameters()) + [self.beta] + [self.lamb] + [self.w1, self.w2]
    ...
    def loss_func(self):
        ...
        # Track learnable parameter values
        ...
        if self.iter % 100 == 0:
            print(
                'Iter %d, Loss: %.5e, Data: %.5e (T:%.5e, F:%.5e), PDE: %.5e (T:%.5e, F:%.5e), IC: %.5e (T:%.5e, F:%.5e), Beta: %.4f' 
                % (self.iter, loss.item(), loss_data.item(), loss_T_data.item(), loss_F_data.item(),
                   loss_pde.item(), loss_T_pde.item(), loss_F_pde.item(),
                   loss_ic.item(), loss_T_ic.item(), loss_F_ic.item(), self.beta.item())
            )
    ...
    def plot_beta_lambda_convergence(self):
        """
        Plot the convergence of beta and lambda parameters during training
        """
        real_beta = fire_params['beta']
        real_lambda = fire_params['lamb']

        final_beta = self.betas[-1] if self.betas else 0.0
        error_percent = abs(final_beta - real_beta) / real_beta * 100

        plt.figure(figsize=(10, 6))
        plt.plot(self.betas, 'b-', linewidth=2, label=f'Estimated β = {final_beta:.4f}')
        plt.axhline(y=real_beta, color='r', linestyle='--', linewidth=2,
                   label=f'True β = {real_beta:.4f} | Rel. error: {error_percent:.2f}%')
        plt.xlabel('Iteration')
        plt.ylabel('Beta Parameter')
        plt.title('Beta Parameter Convergence During Inverse PINN Training')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()

        # Print final estimation accuracy
        print(f"\nBeta Estimation Results:")
        print(f"True value: {real_beta:.4f}")
        print(f"Estimated: {final_beta:.4f}")
        print(f"Error: {error_percent:.2f}%")
        
        plt.savefig('Fire_PINN_2D/graphs/beta_convergence.png', dpi=300, bbox_inches='tight')
        plt.show()


        final_lambda = self.lambs[-1] if self.lambs else 0.0
        error_percent = abs(final_lambda - real_lambda) / real_lambda * 100

        plt.figure(figsize=(10, 6))
        plt.plot(self.lambs, 'b-', linewidth=2, label=f'Estimated λ = {final_lambda:.4f}')
        plt.axhline(y=real_lambda, color='r', linestyle='--', linewidth=2,
                   label=f'True λ = {real_lambda:.4f} | Rel. error: {error_percent:.2f}%')
        plt.xlabel('Iteration')
        plt.ylabel('Lambda Parameter')
        plt.title('Lambda Parameter Convergence During Inverse PINN Training')
        plt.ylim(0, 0.1)
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        ...
        print(f"True value: {real_lambda:.4f}")
        print(f"Estimated: {final_lambda:.4f}")
        print(f"Error: {error_percent:.2f}%")

        plt.savefig('Fire_PINN_2D/graphs/lambda_convergence.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_w_convergence(self):
        ...
        plt.tight_layout()
        plt.savefig('Fire_PINN_2D/graphs/w1_w2_convergence.png', dpi=300, bbox_inches='tight')
        plt.show()
...
print(f"\nFire parameters:")
print(f"beta={beta:.4f}, lamb={lamb:.4f}, Ta={Ta}")
...
layers = [3, 20, 20, 20, 20, 2]  # Network architecture: input=3 (x,y,t), output=2 (T,F), 4 hidden layers of 20 neurons
...
# Randomly sample training data points with same
...
# Select a few time snapshots to visualize
time_indices = [0, nt//4, nt//2, 3*nt//4, nt-1]
time_labels = [f't={t_unique[i]:.2f}' for i in time_indices]

# Create figure with snapshots at different times
...
plt.tight_layout()
plt.savefig('Fire_PINN_2D/graphs/inverse_fire_pinn_2d_snapshots.png', dpi=150, bbox_inches='tight')
print("Snapshots saved to 'inverse_fire_pinn_2d_snapshots.png'")

# Create error visualization snapshots at different times
...
for i, (tidx, tlabel) in enumerate(zip(time_indices, time_labels)):
    ...
    axes2[i, 1].legend(handles=[
        Patch(fc='none', ec='none', label=f'Min:  {np.abs(error_F_grid).min():.4e}'),
        Patch(fc='none', ec='none', label=f'Max:  {np.abs(error_F_grid).max():.4e}'),
        Patch(fc='none', ec='none', label=f'Rel:  {rel_F:.4e}'),
        Patch(fc='none', ec='none', label=f'MSE: {mse_F:.4e}'),
    ], loc='upper right', fontsize=8, framealpha=0.8)
...
plt.tight_layout()
plt.savefig('Fire_PINN_2D/graphs/inverse_fire_pinn_2d_error_snapshots.png', dpi=150, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Fire_PINN_2D/graphs/inverse_fire_pinn_lbfgs_loss_history.png', dpi=150, bbox_inches='tight')
...
model.plot_beta_lambda_convergence()
