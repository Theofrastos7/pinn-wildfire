...
def weighted_laplacian(Z, dx, dy, w1, w2):
    ...
    w1 and w2 can be scalars or arrays (for spatially varying diffusion coefficients)
    """
    ...
    L = np.zeros_like(Z)
    # Handle spatially varying w1 and w2 (if arrays, slice to interior)
    if isinstance(w1, np.ndarray):
        w1_interior = w1[1:-1, 1:-1]
    else:
        w1_interior = w1
    if isinstance(w2, np.ndarray):
        w2_interior = w2[1:-1, 1:-1]
    else:
        w2_interior = w2
    L[1:-1,1:-1] = w1_interior*(Z[2:,1:-1] - 2*Z[1:-1,1:-1] + Z[:-2,1:-1])/dx**2 \
                  + w2_interior*(Z[1:-1,2:] - 2*Z[1:-1,1:-1] + Z[1:-1,:-2])/dy**2
    # Neumann BCs
    L[0,:] = L[1,:]; L[-1,:] = L[-2,:]
    L[:,0] = L[:,1]; L[:,-1] = L[:,-2]
    return L
...
def Diffusion_Reaction_Windless_Operator_Splitting(k, w1_q1, w2_q1, w1_q4, w2_q4, w1_q3, w2_q3, w1_q2, w2_q2,
                                                   A, B, C1, CS1, C4, CS4, C3, CS3, C2, CS2, Ta, Nx, Ny, dx, dy, dt, T_total,
                                                   area_start_x, area_start_y, area_size_x, area_size_y, temp1, temp2, fuel1, fuel2):
    """
    4 quadrants with different w parameters (diffusion weights) and different C/CS parameters (reaction)
    Q1 is top-right, where: x >= mid_x, y >= mid_y = w1_q1, w2_q1, C1, CS1 (lamb1, beta1)
    Q2 is top-left, where:  x < mid_x,  y >= mid_y = w1_q2, w2_q2, C2, CS2 (lamb2, beta2)
    Q3 is bottom-left, where: x < mid_x,  y < mid_y  = w1_q3, w2_q3, C3, CS3 (lamb3, beta3)
    Q4 is bottom-right, where: x >= mid_x, y < mid_y  = w1_q4, w2_q4, C4, CS4 (lamb4, beta4)
    """
    ...
    # Create spatially varying w1 and w2 arrays for 4 each quadrant
    mid_x = Nx // 2 # middle x
    mid_y = Ny // 2 # middle y

    # Initialize w1 and w2 arrays with zeros
    w1_array = np.zeros((Nx, Ny))
    w2_array = np.zeros((Nx, Ny))

    # Q3 bottom-left: low x, low y
    w1_array[:mid_x, :mid_y] = w1_q3
    w2_array[:mid_x, :mid_y] = w2_q3

    # Q4 bottom-right: low y, high x
    w1_array[:mid_x, mid_y:] = w1_q4
    w2_array[:mid_x, mid_y:] = w2_q4

    # Q2 top-left: high y, low x
    w1_array[mid_x:, :mid_y] = w1_q2
    w2_array[mid_x:, :mid_y] = w2_q2

    # Q1 bottom-right: high x (right), high y (top)
    w1_array[mid_x:, mid_y:] = w1_q1
    w2_array[mid_x:, mid_y:] = w2_q1
    ...
    # 1. RK4 diffusion step with spatially varying w
        T_diff = rk4_diffusion(Tn, k, dt, dx, dy, w1_array, w2_array)
