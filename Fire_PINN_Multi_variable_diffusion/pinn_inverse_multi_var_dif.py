...
learning_rate = 0.2 # learning rate for LBFGS
data_points = 10000  # Data points for data
collocation_points = 500  # Collocation points for physics
dataset = 'Fire_PINN_Multi_variable_diffusion/data/fire_dataset_columns.csv'
...
class FirePINN():
    def __init__(self, X_data, TF_data, X_collocation, X_ic, TF_ic, layers, lb, ub, fire_params):
        ...
        lb, ub: domain bounds
        fire_params: dict with Ta, lamb1, beta1, lamb2, beta2, lamb3, beta3, lamb4, beta4, x_mid, y_mid, w1_q1, w2_q1, w1_q2, w2_q2, w1_q3, w2_q3, w1_q4, w2_q4
        """
        # Domain bounds
        ...
        # Learnable parameters lamb1-4, beta1-4, w1_q1-w2_q4
        ...
        self.w1_q1 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))  # top-right
        self.w2_q1 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))
        self.w1_q2 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))  # top-left
        self.w2_q2 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))
        self.w1_q3 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))  # bottom-left
        self.w2_q3 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))
        self.w1_q4 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))  # bottom-right
        self.w2_q4 = torch.nn.Parameter(torch.tensor([1.0], requires_grad=True))
    ...
    def plot_parameter_convergence(self):
        ...
        for idx, (name, pos) in enumerate(zip(quadrant_names, positions)):
            ...
            final_l = lamb_histories[idx][-1] if lamb_histories[idx] else 0.0
            err_l = abs(final_l - true_lambs[idx]) / true_lambs[idx] * 100 if true_lambs[idx] != 0 else 0
            ax.plot(lamb_histories[idx], 'b-', linewidth=2, label=f'Est. λ{idx+1} = {final_l:.4f}')
            ax.axhline(y=true_lambs[idx], color='b', linestyle='--', linewidth=1.5,
                       label=f'True λ{idx+1} = {true_lambs[idx]:.4f} | Err: {err_l:.2f}%')
        ...
        fig_lamb.savefig('Fire_PINN_Multi_variable_diffusion/graphs/lambda_convergence_4quadrants.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        for idx, (name, pos) in enumerate(zip(quadrant_names, positions)):
            ...
            final_b = beta_histories[idx][-1] if beta_histories[idx] else 0.0
            err_b = abs(final_b - true_betas[idx]) / true_betas[idx] * 100 if true_betas[idx] != 0 else 0
            ax.plot(beta_histories[idx], 'r-', linewidth=2, label=f'Est. β{idx+1} = {final_b:.4f}')
            ax.axhline(y=true_betas[idx], color='r', linestyle='--', linewidth=1.5,
                       label=f'True β{idx+1} = {true_betas[idx]:.4f} | Err: {err_b:.2f}%')
            ax.set_xlabel('Iteration', fontsize=11)
        ...
        fig_beta.savefig('Fire_PINN_Multi_variable_diffusion/graphs/beta_convergence_4quadrants.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_w_convergence(self):
        ...
        plt.tight_layout()
        plt.savefig('Fire_PINN_Multi_variable_diffusion/graphs/w_parameters_convergence.png', dpi=300, bbox_inches='tight')
        plt.show()
...
plt.tight_layout()
plt.savefig('Fire_PINN_Multi_variable_diffusion/graphs/inverse_fire_pinn_2d_snapshots.png', dpi=150, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Fire_PINN_Multi_variable_diffusion/graphs/inverse_fire_pinn_2d_error_snapshots.png', dpi=150, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Fire_PINN_Multi_variable_diffusion/graphs/inverse_fire_pinn_lbfgs_loss_history.png', dpi=150, bbox_inches='tight')
print("Loss history saved to 'inverse_fire_pinn_lbfgs_loss_history.png'")

# Save model
torch.save(model.dnn.state_dict(), 'Fire_PINN_Multi_variable_diffusion/data/inverse_fire_pinn_lbfgs_model.pth')
